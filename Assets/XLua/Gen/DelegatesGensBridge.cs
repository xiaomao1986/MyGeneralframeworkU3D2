...
namespace XLua
{
    public partial class DelegateBridge : DelegateBridgeBase
    {
		...
		public void SystemVoid(UnityEngine.GameObject p)
		{
#if THREAD_SAFT || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public void SystemVoid(object self, object a)
		{
#if THREAD_SAFT || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
		...
		public override Delegate GetDelegateByType(Type type)
		{
		    ...
		    if (type == typeof(my_UI.Button.bittonClick))
			{
			    return new my_UI.Button.bittonClick(SystemVoid);
			}
		
		    if (type == typeof(UIManager.testGameObject))
			{
			    return new UIManager.testGameObject(SystemVoid);
			}
		    ...
		    if (type == typeof(testint))
			{
			    return new testint(SystemVoid);
			}
		
		    if (type == typeof(testGameObject))
			{
			    return new testGameObject(SystemVoid);
			}
		
		    if (type == typeof(bittonClick))
			{
			    return new bittonClick(SystemVoid);
			}
		
		    if (type == typeof(test01))
			{
			    return new test01(SystemVoid);
			}
